  ...
  if vRP.hasPermission({user_id, "#"..cfg.list[id].key..".>0"}) or vRP.hasPermission({user_id,cfg.list[id].permission}) then
    ...
    SetTimeout(0, function()
      ...
      if cfg.list[id].locked then
		...
		TriggerClientEvent("pNotify:SendNotification",player,{text = "A PORTA ESTÁ TRANCADA | <span color='red'>" ..cfg.list[id].name.. "</span> ", type = "info", timeout = (3000),layout = "centerRight", theme = "semanticui"})
      else
		...
		TriggerClientEvent("pNotify:SendNotification",player,{text = "SUA CREDENCIAL ABRIU A PORTA | <span color='red'>" ..cfg.list[id].name.. "</span> ", type = "info", timeout = (3000),layout = "centerRight", theme = "semanticui"})
      end
    end)
  else
	...
	TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>CREDENCIAL INVALIDA </span>", type = "error", timeout = (3000),layout = "centerRight", theme = "semanticui"})
  end
end)
