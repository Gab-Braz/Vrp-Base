...
local lang = {
  ...
  money = {
    display = "<a class='money'><span><i class=\"fas fa-dollar-sign\"></i> {1}</span></a>",
    given = "Enviar ~r~{1}$.",
  ...
  atm = {
    display = "<a class='bank'><span><i class=\"far fa-credit-card\"></i> {1}</span></a>",
  ...
  cityhall = {
    ...
    menu = {
      title = "Identidade",
      info = "<em>Nome: </em>{1}<br /><em>Sobrenome: </em>{2}<br /><em>Idade: </em>{3}<br /><em>RG n°: </em>{4}<br /><em>Telefone: </em>{5}<br /><em>Endereço: </em>{7}, {6}</br><em>Emprego: </em>{8}</br></em>Carteira: </em>{9}</br><em>Passaporte: </em>{10}</br><em>CNH: </em>{11}</br><em>Porte: </em>{12}</br><em>VIP: </em>{13}</br>"
    }
  },
  police = {
    title = "Interação PM",
  ...
  phone = {
    ...
    service = {
      ...
      ask_call = "Chamada Recebida de {1}, você deseja aceitar este Serviço? <em>{2}</em> ",
