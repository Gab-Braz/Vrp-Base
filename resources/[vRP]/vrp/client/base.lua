...
function tvRP.playerIsNearOtherPlayer(id, radius)
	local monid = PlayerId()
	local sonid = GetPlayerFromServerId(id)
	if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(monid)), GetEntityCoords(GetPlayerPed(sonid)), true) < radius+0.001 then
		return true
	else
		return false
	end
...
function tvRP.playerIsNearCoords(px,py,pz,radius)
	local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
	local distance = GetDistanceBetweenCoords(x,y,z,px,py,pz,true)
	if distance <= radius then
		return true
...
function tvRP.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,dict,x+0.0001,y+0.0001,z+0.0001,name,0,range+0.0001,0)
end

-- play sound
function tvRP.playSound(dict,name)
  PlaySound(-1,dict,name,0,0,1)
end
...
RegisterNetEvent("b2k:blackout")
AddEventHandler("b2k:blackout", function(cond)
	local status = false
	if cond == 1 then
		status = true
	end
...
RegisterNetEvent("b2k:lightning")
AddEventHandler("b2k:lightning", function(vezes)
