    ...
    if v > 0 then
      ...
      if user_id ~= nil then
        if vRP.tryDeposit(user_id,v) then
          ...
          vRPclient.notify(player, {"~r~Depositado " ..v.. "R$"})
        else
    ...
    if v > 0 then
      ...
      if user_id ~= nil then
        if vRP.tryWithdraw(user_id,v) then
          ...
          vRPclient.notify(player, {"~r~Retirado " ..v.. "R$"})
        else
...
local function build_client_atms(source)
  ...
  if user_id ~= nil then
    for k,v in pairs(atms) do
      ...
      vRPclient.addBlip(source,{x,y,z,108,25,lang.atm.title()})
      vRPclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,0,255,125,125,150})
      vRPclient.addTargetMarker(source,{29,x,y,z,1.0,1.0,1.0,0,255,125,125,150})
