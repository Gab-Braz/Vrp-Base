...
local function leave_slot(user_id,player,stype,sid) -- called when a player leave a slot
  vRP.prompt(player,"Você deseja sair dessa casa? digite sim ou não.","",function(player,resposta)
    if resposta == "sim" then
      ...
      vRPclient.notify(player,{"~r~Você não saiu da casa."})
    end
  end)
end
  ...
  vRP.getUserAddress(user_id, function(address)
    ...
    for k,v in pairs(cfg.slot_types[stype][sid]) do
      ...
      if name == "entry" then
        ...
        local nid = "vRP:home:slot"..stype..sid
        vRPclient.setNamedMarker(player,{nid,x,y,z,1.5,1.5,0.5,0,255,125,125,150})
  ...
  menu[lang.home.buy.title()] = {function(player,choice)
    vRP.getUserAddress(user_id, function(address)
      if address == nil then -- check if not already have a home
        vRP.prompt(player,"Você deseja comprar essa casa ? digite sim ou não","",function(player,resposta)
          if resposta == "sim" then
            ...
            vRPclient.notify(player,{"~r~Você não comprou essa casa."})
          end
        end)
      else
  ...
  menu[lang.home.sell.title()] = {function(player,choice)
    vRP.getUserAddress(user_id, function(address)
      if address ~= nil and address.home == home_name then -- check if already have a home
        -- "Deseja vender a casa? Digite sim ou nao:"

        vRP.prompt(player,"Deseja vender a casa? Digite sim ou nao:","",function(player,cbd)

        if cbd == "sim" then 
            vRP.removeUserAddress(user_id)
            vRPclient.notify(player,{"~r~Aguarde, estamos procurando compradores..."})
            Wait(5000)
            vRPclient.notify(player,{"~r~A sua casa foi comprada pelo prefeito!"})
            Wait(2000)
            vRP.giveMoney(user_id, home.sell_price)
        else
            vRPclient.notify(player,{"~r~A casa não foi vendida"})
        end
...
local function build_client_homes(source)
  ...
  if user_id ~= nil then
    for k,v in pairs(cfg.homes) do
      ...
      if string.match(k, "Barraco") then
        vRPclient.addBlip(source,{x,y,z,374,1,"Barraco"})
      elseif string.match(k, "Casa") or string.match(k, "Luxo") or string.match(k, "Basico")  then
        vRPclient.addBlip(source,{x,y,z,374,2,"Casa"})
      elseif string.match(k, "Triplex") or string.match(k, "Mansão") then
        vRPclient.addBlip(source,{x,y,z,492,5,"Casa de Luxo"})
      elseif string.match(k,"Apartamento") then
        vRPclient.addBlip(source,{x,y,z,492,5,"Apartamentos"})
  	  else
        vRPclient.addBlip(source,{x,y,z,v.blipid,v.blipcolor,k})
  	  end
      vRPclient.addMarker(source,{x,y,z,1.0,1.0,1.0,0,255,125,125,150})
