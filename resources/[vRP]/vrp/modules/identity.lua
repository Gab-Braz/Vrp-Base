                ...
                if vRP.tryPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
                      ...
                      vRPclient.notify(player, {"~r~Pagou " ..cfg.new_identity_cost.. "R$"})
                    end)
                  end)
                else
...
--           end
--           -- get wallet amount
--           local walletamount = format_thousand(math.floor(vRP.getMoney(user_id)))
--           local cnh = ""
...
--           end
--           -- get bank amount
--           local content = lang.cityhall.menu.info({htmlEntities.encode(identity.name),htmlEntities.encode(identity.firstname),identity.age,identity.registration,identity.phone,home,number,job,walletamount,user_id,cnh,porte,vip})
--           local choices = {}
