...
function vRP.defInventoryItem(idname,name,description,choices,weight,itemlistname)
  if weight == nil then
    weight = 0
  end

  if itemlistname == nil then
    itemlistname = name
  end

  local item = {name=name,description=description,choices=choices,weight=weight,itemlistname=itemlistname}
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                vRPclient.playAnim(nplayer,{true,{{"mp_common","givetake2_a",1}},false})
        				...
        				if string.match(idname, "wbody") or string.match(idname, "wammo") then
        					TriggerClientEvent("amorim:pessoalLog", source, "De: "..user_id.." - Para: "..nuser_id.." - Item: " ..idname.." - Amount: " .. amount)
        				end
        				
        				if string.match(idname, "money") then
        					TriggerClientEvent("amorim:pessoalLog", source, "De: "..user_id.." - Para: "..nuser_id.." - Item: " ..idname.." - Amount: " .. amount)
        				end
...
function vRP.computeItemListName(item,args)
  if type(item.itemlistname) == "string" then return item.itemlistname
  else return item.itemlistname(args) end
end
...
function vRP.getItemDefinition(idname)
  ...
  if item ~= nil then
    return vRP.computeItemName(item,args), vRP.computeItemDescription(item,args), vRP.computeItemWeight(item,args), vRP.computeItemListName(item,args)
  end
  ...
  if user_id ~= nil then
    ...
    if data then
      ...
      for k,v in pairs(data.inventory) do 
        ...
        if name ~= nil then
          kitems[itemlistname] = k -- reference item by display name
          menudata[itemlistname] = {choose,lang.inventory.iteminfo({v.amount,description,string.format("%.2f",weight)})}
        end
      end
...
local function build_itemlist_menu(name, items, cb)
  ...
  for k,v in pairs(items) do 
    ...
    if name ~= nil then
      kitems[itemlistname] = k -- reference item by display name
      menu[itemlistname] = {choose,lang.inventory.iteminfo({v.amount,description,string.format("%.2f", weight)})}
    end
  end
                ...
                if new_weight <= vRP.getInventoryMaxWeight(user_id) then
                  ...
                  if citem.amount <= 0 then
                    chest.items[idname] = nil -- remove item entry
        				  ...
        				  if string.match(idname, "wbody") or string.match(idname, "wammo") then
        					TriggerClientEvent("amorim:pessoalLog", source, "Baú","Retirado Por: "..user_id.." - ChestName: "..name.." - Item: " .. idname.." - Amount: " .. amount)
        				  end
                ...
                if amount > 0 and vRP.tryGetInventoryItem(user_id, idname, amount, true) then
                  ...
                  if citem ~= nil then
                    ...
                    chest.items[idname] = {amount=amount}
        				  ...
        				  if string.match(idname, "wbody") or string.match(idname, "wammo") then
                  --vRP.logToFile("chestVehLog.txt","Colocado Por: "..user_id.." - ChestName: "..name.." - Item: " ..idname.." - Amount: " ..amount)
                  TriggerClientEvent("amorim:pessoalLog", source, "Baú","Colocado Por: "..user_id.." - ChestName: "..name.." - Item: " ..idname.." - Amount: " ..amount)
        				  end
        				  
        				  if string.match(idname, "money") then
                    TriggerClientEvent("amorim:pessoalLog", source, "Colocado Por: "..user_id.." - ChestName: "..name.." - Item: " ..idname.." - Amount: " ..amount)
        				  end
...
-- STATIC PERSONAL CHESTS

local function build_client_static_personal_chests(source)
  local user_id = vRP.getUserId(source)
  if user_id then
    for k,v in pairs(cfg.static_personal_chests) do
      local mtype,x,y,z = table.unpack(v)
      local schest = cfg.static_personal_chest_types[mtype]
      ...
      if schest then
        local function schest_enter(source)
          ...
          if user_id ~= nil and vRP.hasPermissions(user_id,schest.permissions or {}) then
            -- open chest
            vRP.openChest(source, "static_personal:"..schest.id.."_u"..user_id, schest.weight or 0)
          end
        end
        ...
        vRPclient.addBlip(source,{x,y,z,schest.blipid,schest.blipcolor,schest.title})
        vRPclient.addMarker(source,{x,y,z-1.0,1.5,1.5,0.7,255,226,0,125,150})

        vRP.setArea(source,"vRP:static_pc:"..schest.id.."_u"..user_id,x,y,z,1,1.5,schest_enter,schest_leave)
      end
    end
  end
end
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    -- load static personal chests
    build_client_static_personal_chests(source)
  end
end)