...
RegisterNetEvent("silenciador")
AddEventHandler("silenciador", function()
	...
	if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
        ...
        notificar("~g~Você equipou o silenciador na arma escolhida.")
	...
	elseif currentWeaponHash == GetHashKey("WEAPON_SNIPERRIFLE") then
		...
		notificar("~r~A arma escolhida não tem como usar silenciador.")		
    end
end)

RegisterNetEvent("lanterna")
AddEventHandler("lanterna", function()
    local ped = PlayerPedId()
    local currentWeaponHash = GetSelectedPedWeapon(ped)

    if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
        GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey("WEAPON_PISTOL"), GetHashKey("COMPONENT_AT_PI_FLSH"))
        notificar("~g~Você equipou lanterna na sua arma.")
   ...
   elseif currentWeaponHash == GetHashKey("WEAPON_MARKSMANRIFLE") then
        ...
        notificar("~r~A arma selecionada não possui suporte para lanterna.")
    end
end)

RegisterNetEvent("grip")
AddEventHandler("grip", function()
     local ped = PlayerPedId()
     local currentWeaponHash = GetSelectedPedWeapon(ped)
     ...
     if currentWeaponHash == GetHashKey("WEAPON_COMBATPDW") then
          ...
          notificar("~g~Você equipou Grip na sua arma.")  
    elseif currentWeaponHash == GetHashKey("WEAPON_ASSAULTRIFLE") then
    ...
    elseif currentWeaponHash == GetHashKey("WEAPON_MARKSMANRIFLE") then
          ...
          notificar("~r~A arma selecionada não possui suporte para o grip.")
     end
end)
...
AddEventHandler("skin", function()
     ...
     if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
          ...
          notificar("~g~Você colocou a skin na sua arma.")  
    elseif currentWeaponHash == GetHashKey("WEAPON_PISTOL50") then
    ...
    elseif currentWeaponHash == GetHashKey("WEAPON_ADVANCEDRIFLE") then
          ...
          notificar("~r~A arma selecionada não possui suporte para a skin.")
     end
end)