...
RegisterServerEvent("bmg:sendLog")
AddEventHandler("bmg:sendLog",function(name,message)
	if message == nil or message == '' or message:sub(1, 1) == '/' then return FALSE end
	PerformHttpRequest('https://discordapp.com/api/webhooks/568322511972859924/HyHb5rzG4H_yGPZB2gy0mMmX3jvZbA6bswYsO0YZz9EhRMzk4VH0vokbDcz4oPAtJw7j', function(err, text, headers) end, 'POST', json.encode({username = name, content = message}), { ['Content-Type'] = 'application/json' })
end)
...
AddEventHandler('es_bank:toofar', function(robb)
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('chatMessage', -1, '@ANONIMO', { 105, 105, 105 }, "Os assaltantes fugiram do ^2" .. banks[robb].nameofbank ..".")
	end
end)
  ...
  if vRP.hasPermission({user_id,cfg.permission}) then
    ...
    if #cops >= cfg.cops then
	  if banks[robb] then
		  ...
		  TriggerClientEvent('chatMessage', -1, 'NOTÍCIAS', { 128, 0, 0 }, "Está acontecendo um assalto no ^2" .. bank.nameofbank)
			TriggerClientEvent('chatMessage', player, 'AVISO', { 0, 0x99, 255 }, "Você começou um assalto no ^2" .. bank.nameofbank .. "^0, cuide do cofre até ele abrir, não saia da área!")
				TriggerEvent("bmg:sendLog", "Assalto", "ID: "..user_id.." ("..GetPlayerName(player)..") começou o assalto ao banco "..bank.nameofbank)
			  ...
			  if(robbers[savedSource])then
				  if(user_id)then
					  vRP.giveInventoryItem({user_id,"dirty_money",bank.reward,true})
						TriggerClientEvent('chatMessage', -1, 'NOTÍCIAS', { 128, 0, 0 }, "O roubo terminou no ^2" .. bank.nameofbank .. "^0!")
						TriggerEvent("bmg:sendLog", "Assalto", "ID: "..user_id.." ("..GetPlayerName(player)..") assaltou ao banco "..bank.nameofbank.." e conseguiu roubar "..bank.reward)
