...
function ds(name, message)
  ...
  PerformHttpRequest('https://discordapp.com/api/webhooks/555927978287169536/wsHTx-jiqFAYD-WCwDDCFMdCVIpCU6u5hzPpHV3ptV4bgUN7V7oH-KUazPkWTmQgeXgV', function(err, text, headers) end, 'POST', json.encode({username = name, content = message}), { ['Content-Type'] = 'application/json' })
end
...
local choice_store_money = {function(player, choice)
  ...
  if user_id ~= nil then
    ...
    if vRP.tryPayment({user_id, amount}) then -- unpack the money
      ...
      ds("MONEY", "ID: "..user_id.." ("..GetPlayerName(player).."), guardou no inventario "..amount)
    end
  end
        ...
        vRPclient.isInComa(nplayer,{}, function(in_coma)
          if in_coma then
            ...
            SetTimeout(15000, function()
              ...
              ds("BASIC_MENU", "ID: "..user_id.." ("..GetPlayerName(player)..") looteou o ID: "..nuser_id..(" ("..GetPlayerName(nplayer)..")."))
	              end
				end
			  end
  ...
  if user_id ~= nil then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
      if nplayer ~= nil then
        ...
        ds("BASIC_MENU", "ID: "..user_id.." ("..GetPlayerName(player)..") algemou o ID: "..nuser_id..(" ("..GetPlayerName(nplayer)..")."))
			else
...
local choice_store_armor = {function(player, choice)
 local user_id = vRP.getUserId({player})
 if user_id ~= nil then
  vRP.prompt({player,"Deseja guardar o colete ? Digite sim ou não", "",function(player,resposta)
    if string.lower(resposta) == "sim" then
      ...
      if string.lower(resposta) == "nao" then
        vRPclient.notify(player,{"~r~Você não guardou o colete."})
      end
    end
    end})
  end
	...
	if user_list ~= "" then
	  vRP.prompt({player,"Jogadores perto:" .. user_list,"",function(player,target_id) 
	    if target_id ~= nil and target_id ~= "" then 
          ...
          --ds("BASIC_MENU", "ID: "..user_id.." ("..GetPlayerName(player)..") preendeu o ID: "..target_id.." ("..GetPlayerName(target_id)..") por " ..jail_time.. ".")
			      else
	...
	vRP.prompt({player,"Player ID:","",function(player,target_id) 
	  if target_id ~= nil and target_id ~= "" then 
		vRP.getUData({tonumber(target_id),"vRP:jail:time",function(value)
          ...
          --ds("BASIC_MENU", "ID: "..user_id.." ("..GetPlayerName(player)..") liberou da cadeia o ID: "..target_id.." ("..GetPlayerName(target_id)..") na sentença de " ..custom.. " minutos.")
				else
				  vRPclient.notify(player,{"~r~Essa ID parece inválida."})
				end
			  else
	  ...
	  vRP.getUserByPhone({phone, function(target_id)
		if target_id~=nil then
			if charge ~= nil and charge ~= "" then 
              ...
              ds("BASIC_MENU", "ID: "..user_id.." ("..GetPlayerName(player)..") pagou pelo celular pro ID: "..target_id.." ("..GetPlayerName(target_id).."), novo saldo do pagador "..my_bank.." saldo do que recebeu "..target_bank..".")
					    vRP.closeMenu({player})
                      else
	...
	if transfer ~= nil and transfer ~= "" and tonumber(transfer)>0 then 
	  vRP.getUserByPhone({phone, function(target_id)
        ...
        --vRPbm.logInfoToFile("mpayLog.txt",user_id .. " pagou por telefone "..target_id.." a quantidade de " .. transfer .. ", pagamento do banco do usuário para "..user_id.." é igual a $"..my_bank.." e para "..user_id.." é igual a $"..target_bank)
          --ds("BASIC_MENU", "ID: "..user_id.." ("..name..") pagou por telefone o ID: "..target_id.." ("..GetPlayerName(target_id)..") a quantidade de " .. transfer .. ", pagamento do banco do usuário para "..user_id.." é igual a $"..my_bank.." e para "..user_id.." é igual a $"..target_bank)
			  vRP.getUserIdentity({user_id, function(identity)
		        local my_directory_name = vRP.getPhoneDirectoryName({target_id, identity.phone})
			    if my_directory_name == "unknown" then
		          my_directory_name = identity.phone
...
local ch_spawnveh = {function(player,choice) 
  vRP.prompt({player,"Vehicle Model:","",function(player,model)
    local user_id = vRP.getUserId({player})
      ...
      ds("BASIC_MENU", "ID: "..user_id.." ("..GetPlayerName(player).."), spawnou pelo admin o veículo: "..model)
	  else
...
vRP.registerMenuBuilder({"unimed", function(add,data)
        ...
        if nuser_id ~= nil then
		  ...
		  if math.random(1,3) == 1 then
            if vRP.tryPayment({nuser_id,amount}) then
              vRPclient.notify(nplayer,{"Assaltou ~r~"..amount.."R$."})
		      vRP.giveInventoryItem({user_id,"dirty_money",amount,true})
            else
              vRPclient.notify(player,{lang.money.not_enough()})
            end
		  else
            vRPclient.notify(nplayer,{"~g~Não conseguiram te assaltar."})
            vRPclient.notify(player,{"~r~Tentativa de assaltou falhou."})
		  end
        else
          vRPclient.notify(player,{lang.common.no_player_near()})
        end
...
vRP.registerMenuBuilder({"pessoal", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission({user_id,"carjacker.lockpick"}) then
      ...
      choices["Desbugar Rosto"] = ch_fixhair -- just a work around for barbershop green hair bug while I am busy

      choices["Lista de players"] = ch_userlist -- a user list for players with vRP ids, player name and identity names only.

      choices["Guardar Colete"] = choice_store_armor -- store player armor
    add(choices)
  end
end})
...
vRP.registerMenuBuilder({"policia", function(add, data)
  local user_id = vRP.getUserId({data.player})
    ...
    if vRP.hasPermission({user_id,"police.spikes"}) then
      choices["Tapete de Pregos"] = ch_spikes -- Toggle spikes
    end
