...
AddEventHandler('es_holdup:toofar', function(robb)
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('chatMessage', -1, '@ANONIMO', { 105, 105, 105 }, "Os assaltantes fugiram da ^2" .. stores[robb].nameofstore..".")
	end
end)
  ...
  if vRP.hasPermission({user_id,cfg.permission}) then
    ...
    if #cops >= cfg.cops then
	  if stores[robb] then
		  ...
		  TriggerClientEvent('chatMessage', -1, 'NOTÍCIAS', { 128, 0, 0 }, "Está acontecendo um assalto na ^2" .. store.nameofstore)
		  ...
		  SetTimeout(cfg.seconds*1000, function()
			  if(robbers[savedSource])then
				  if(user_id)then
					  ...
					  TriggerClientEvent('chatMessage', -1, 'NOTÍCIAS', { 128, 0, 0 }, "O roubo terminou na ^2" .. store.nameofstore .. "^0!")
