		...
		if not InBlacklistedVehicle then
			...
			if nearPump then
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z']+0.3, "Saia do veículo para comprar/abastecer o galão.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z']+0.3, "Pressione ~g~E~w~ para comprar/abastecer o galão. ~r~R$200.00~w~/galão | ~r~R$150.00~w~/abastecer.")

					if IsControlJustReleased(0, 46) then
						TriggerServerEvent('LegacyFuel:buyCan', Config.petrolCanPrice)
					end
				end
			end
...
function DisplayHud()
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		...
		if rpm > 0.1 then
			DrawAdvancedText(1.072 - UI.x, 0.98 - UI.y, 0.005, 0.0028, 0.6, fuel.."%", 200, 200, 200, 255, 4, 1)
		end
	end
end
