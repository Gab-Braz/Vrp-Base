    ...
    if tattoo then
	  ...
	  if tattoo == "CLEAR" then-- get player weapons to not rebuy the body
        ...
        if user_id ~= nil and vRP.tryFullPayment({user_id,price}) then
          ...
          vRPclient.notify(player,{"Você pagou ~r~R$ "..price..""})
        else
		...
		vRP.getUData({user_id,"vRP:tattoos",function(value)
          ...
          vRPclient.notify(player,{"~y~Você já possui essa tatuagem"})
		  end
		end})
	  end
    end
  ...
  if user_id ~= nil then
    for k,v in pairs(cfg.shops) do
      ...
      if group and menu then
        ...
        vRPclient.addMarker(source,{x,y,z-1,3,3,0.5,0,255,125,125,150})

        vRP.setArea({source,"vRP:tattooshop"..k,x,y,z,3,1.5,tattooshop_enter,tattooshop_leave})
      end
    end
  end
end
