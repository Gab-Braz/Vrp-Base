			...
			if(theTrailer ~= nil)then
				if not (IsEntityDead(theTrailer)) then
					...
					if (ok) then
						...
						trucker_drawTxt("~r~[EMPRESA] ~w~Voce precisa pegar a ~r~Carga ~w~para terminar a Entrega!",1,1,0.5,0.9,0.8,255,255,255,255)
					end
					...
					elseif(isInVehicle == true)then
						if not IsEntityAttached(theTrailer) then
							...
							trucker_drawTxt("~r~Destino: ~w~"..destinationName,1,1,0.5,0.85,0.8,255,255,255,255)
							trucker_drawTxt("~r~Carga: ~w~"..destinationType,1,1,0.5,0.90,0.8,255,255,255,255)
							trucker_drawTxt("~r~Dano a Carga: ~w~"..GetEntityHealth(theTrailer).." ~w~de Vida",4,1,0.5,0.95,0.5,255,255,255,255)
						end
					end
				else
					finishJob()
					vRP.notify({"~w~[EMPRESA] ~r~A Entrega foi Cancelada!"})
	...
	while true do
		...
		if(inJob == true)then
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x3, y3, z3) < 40.0)then
				destinationName = jobDetails[1]
				DrawText3D(x3, y3, z3+0.4, "~r~"..destinationName, 2.0, 1)
		...
		if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 40.0)then
			DrawText3D(x,y,z+0.4, "~r~Empresa de Caminhao", 2.0, 1)
			DrawText3D(x,y,z+0.15, "~w~Transporte de Mercadorias Importadas", 1, 7)
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, x, y, z) < 5)then
				...
				if (incircle == true) then
					...
					if (hasTruck == 1) then
						if(inJob == false)then
							trucker_drawTxt("~r~[EMPRESA] ~w~Pressione ~r~[E] ~w~para acessar o Menu de Entrega!",1,1,0.5,0.8,0.8,255,255,255,255)
						else
							trucker_drawTxt("~r~[EMPRESA] ~w~Voce ja tem uma Entrega Pendente!",1,1,0.5,0.74,0.8,255,255,255,255)
						end	
						if(IsControlJustReleased(1, 51)) and (inJob == false)then
							TriggerServerEvent('openTruckerJobs')
						end
					else
						trucker_drawTxt("~r~[EMPRESA] ~w~Voce precisa de um ~r~Caminhao ~w~para fazer uma Entrega!",1,1,0.5,0.8,0.8,255,255,255,255)
					end
				end
